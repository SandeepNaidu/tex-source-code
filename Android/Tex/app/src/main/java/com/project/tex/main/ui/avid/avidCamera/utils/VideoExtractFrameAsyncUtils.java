...
public class VideoExtractFrameAsyncUtils {
    ...
    public void getVideoThumbnailsInfoForEdit(Uri videoPath, String OutPutFileDirPath, long startPosition, long endPosition, int thumbnailsCount) {
        MediaMetadataRetriever metadataRetriever = new MediaMetadataRetriever();
        metadataRetriever.setDataSource(GlobalApplication.Companion.getInstance(), videoPath);
    ...
    private String extractFrame(MediaMetadataRetriever metadataRetriever, long time, String OutPutFileDirPath) {
        ...
        if (bitmap != null) {
            //If the bitmap is scaled, the size of the obtained image is small (about 1kb)
            // but it will be very blurry. If the size of the image is not scaled, the image size
            // is about 60kb and the image is very clear
//            Bitmap bitmapNew= scaleImage(bitmap);
            Bitmap bitmapNew = bitmap;
            String path = VideoUtil.saveImageToSDForEdit(bitmapNew, OutPutFileDirPath,
                    System.currentTimeMillis() + "_" + time + VideoUtil.POSTFIX);
    ...
    /**
     * Set a fixed width, and the height will change accordingly, so that the picture will not be deformed
     *
     * @param bm Bitmap
     * @return Bitmap
     */
