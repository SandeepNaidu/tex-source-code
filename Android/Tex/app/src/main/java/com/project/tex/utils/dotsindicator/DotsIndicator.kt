package com.project.tex.utils.dotsindicator
...
class DotsIndicator : LinearLayout {
    ...
    private var dotHeight: Int = ViewUtils.dpToPx(7)
    private var dotWidth: Int = ViewUtils.dpToPx(7)
    var firstDotHeight: Int = ViewUtils.dpToPx(14)
    var firstDotWidth: Int = ViewUtils.dpToPx(14)
    var marginsBetweenDots: Int = ViewUtils.dpToPx(17)
    ...
    var firstSelectedDotResource: Int = R.drawable.circle_white
    var firstUnselectedDotResource: Int = R.drawable.circle_gray
    ...
    fun setDotSelection(position: Int) {
        ...
        newSelection.setImageResource(if (newSelection.tag == 0) firstSelectedDotResource else selectedDotResource)
        selectedDot.setImageResource(if (selection == 0) firstUnselectedDotResource else unselectedDotResource)
        selection = newSelection.tag as Int
    }

    fun setDotProgressiveSelection(position: Int) {
        if (position == selection)
            return
        val newSelection: ImageView = findViewById(position)
        val selectedDot: ImageView = findViewWithTag(selection)
        ...
        for (i in 0 until dotsCount) {
            val eachDot: ImageView? = findViewById(i)
            if (i <= position) {
                eachDot?.setImageResource(if (newSelection.tag == 0) firstSelectedDotResource else selectedDotResource)
            } else {
                eachDot?.setImageResource(if (newSelection.tag == 0) firstUnselectedDotResource else unselectedDotResource)
            }
        }
