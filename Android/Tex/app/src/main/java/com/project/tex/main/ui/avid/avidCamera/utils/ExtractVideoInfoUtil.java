...
public class ExtractVideoInfoUtil {
    ...
    public ExtractVideoInfoUtil(Uri path) {
        ...
        if (file != null && !file.exists()) {
            file = FileUtils.getFile(GlobalApplication.Companion.getInstance(), path);
            if (file != null && !file.exists()) {
                throw new RuntimeException("path file does not exists !");
            }
        }
...
//        Bitmap bitmap = mMetadataRetriever.getFrameAtTime(timeMs * 1000, MediaMetadataRetriever.OPTION_CLOSEST);
    ...
    public String getVideoLength() {
        ...
        fileLength = TextUtils.isEmpty(len) ? 0 : Long.parseLong(len);
        return len;
    }
