    ...
    public void doStartPreview(SurfaceHolder holder, float screenProp) {
        ...
        if (mCamera != null) {
            try {
                ...
                mCamera.setDisplayOrientation(cameraAngle);//viewing angle
                mCamera.setPreviewCallback(this); //Callback every frame
...
//        }
        ...
        if (SELECTED_CAMERA == CAMERA_FRONT_POSITION) {
            //Mobile phone preview handstand processing
            if (cameraAngle == 270) {
                //Horizontal screen
                if (nowAngle == 0) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            File folder = new File(context.getCacheDir(), "video_capture");
            if (!folder.exists()) {
                folder.mkdirs();
            }
            Uri f = Uri.fromFile(new File(folder.getAbsolutePath() + File.separator + videoFileName));
            File nn = new File(f.getPath());
            boolean n = false;
            try {
                n = nn.createNewFile();
            } catch (IOException e) {
            ...
            if (n) {
                videoFileAbsPath = nn.getAbsolutePath();
            }
        }
    ...
    public void stopRecord(boolean isShort, StopRecordCallback callback) {
        ...
        if (mediaRecorder != null) {
            ...
            doStopPreview();
            String fileName = videoFileAbsPath;
